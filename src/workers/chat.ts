...
// True for 08:00-12:00 or 13:30-18:00 on Monday-Friday, Vietnam time. Exported for tests.
export function isWithinTicketWindow(now: Date): boolean {
  const local = new Date(now.getTime() + VIETNAM_OFFSET_MS)
  const day = local.getUTCDay()
  const mins = local.getUTCHours() * 60 + local.getUTCMinutes()
  const inWindow = (mins >= 480 && mins < 720) || (mins >= 810 && mins < 1080)
  return day >= 1 && day <= 5 && inWindow
}
...
// GOOGLE_CHAT_WEBHOOK) only within the weekday 08:00-12:00 / 13:30-18:00 Vietnam window, everything else to

...
- **DO in tests**: `DurableObject` is a type-only interface in `@cloudflare/workers-types` (no runtime value under vitest). `SyncCoordinator` carries its own `ctx`/`env` and implements `Rpc.DurableObjectBranded` so `DurableObjectNamespace<SyncCoordinator>` types the RPC methods; it's unit-tested with a fake storage (`tests/syncCoordinator.test.ts`).
